const url="https://raw.githubusercontent.com/mkatay/booksJson/refs/heads/main/data.json"
...
function renderData(data){
    ...
    authors=getUniqueValues(data,'author')
    //console.log(authors);
...
function getUserOption(e){
    ...
    if(userOption=='author') renderCheckboxes(authors)
}
...
function handleClick(){
    ...
    const filteredBooks=books.filter(obj=>checkedValues.includes(obj[userOption]))
    console.log(filteredBooks);
    //cards:
    document.querySelector('.cards').innerHTML=''
    filteredBooks.forEach(obj=>
        document.querySelector('.cards').innerHTML+=`  
<a href="#" class="block max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
    ...
    <p class="font-normal text-gray-700 dark:text-gray-400">${obj.author} ${obj.year}</p>
</a>
        `
    )
}